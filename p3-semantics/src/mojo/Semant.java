    ...
    Type Check(Type t) {
        class Visitor implements Type.Visitor<Void> {
            ...
            public Void visit(Type.Object t) {
                ...
                final Set<String> methodNames = t.methods.stream()
                        .map((final Value.Method method) -> method.name)
                        .collect(Collectors.toSet());
                for (final Value.Field field : t.fields) {
                    if (methodNames.contains(field.name)) {
                        Error.ID(field.token, "field and method with the same name");
                    }
                }
