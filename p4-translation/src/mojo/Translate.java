    ...
    Tree.Exp Compile(Expr e) {
        class Visitor implements Expr.Visitor<Tree.Exp> {
            ...
            public Tree.Exp visit(Expr.Equal e) {
                ...
                Tree.Stm stm;
                switch (e.op) {
                    case EQ -> stm = BEQ(l, r, t, f);
                    case NE -> stm = BNE(l, r, t, f);
                    default -> {
                        assert false;
                        stm = null;
                    }
            ...
            public Tree.Exp visit(Expr.Compare e) {
                // TODO [DONE]
                final Tree.Exp l = Compile(e.left);
                final Tree.Exp r = Compile(e.right);
                final Label t = new Label();
                final Label f = new Label();
                final Temp temp = new Temp();
                final Tree.Stm stm = switch (e.op) {
                    case GE -> BGE(l, r, t, f);
                    case GT -> BGT(l, r, t, f);
                    case LE -> BLE(l, r, t, f);
                    case LT -> BLT(l, r, t, f);
    ...
    Tree.Stm CompileBranch(Expr e, final Label t, final Label f) {
        class Visitor implements Expr.Visitor<Tree.Stm> {
            ...
            public Tree.Stm visit(Expr.Or e) {
                // TODO [DONE]
                final Label f1 = new Label();
                return SEQ(
                        BEQ(Compile(e.left), CONST(0), f1, t),
                        LABEL(f1),
                        BEQ(Compile(e.right), CONST(0), f, t)
                );
            }
            public Tree.Stm visit(Expr.And e) {
                // TODO [DONE]
                final Label t1 = new Label();
                return SEQ(
                        BEQ(Compile(e.left), CONST(0), f, t1),
                        LABEL(t1),
                        BEQ(Compile(e.right), CONST(0), f, t)
                );
            }
            public Tree.Stm visit(Expr.Not e) {
                // TODO [DONE]
                return BEQ(Compile(e.expr), CONST(0), t, f);
            }
